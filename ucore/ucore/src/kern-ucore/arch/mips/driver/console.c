...
static void serial_init(void)
{
	...
	//TODO
        outb(UART_BASE + lcr, 0x80);// LCR[7]  is 1
        outb(UART_BASE + dll, 27);// DLL msb. 115200 at 50MHz. Formula is Clk/16/baudrate. From axi_uart manual.
        outb(UART_BASE + lcr, 0x00);// DLL lsb.
        outb(UART_BASE + lcr, 0x03);// LCR register. 8n1 parity disabled
        outb(UART_BASE + lcr, 0x00);// IER register. disable interrupts
#endif
...
#elif defined MACH_FPGA
	//TODO
	while ((inw(UART_BASE + 0x04) & 0x01) == 0) ;
	//outw(COM1 + 0x00, c & 0xFF);
	//delay();
	outb(UART_BASE + thr, c & 0xFF);
#endif
}
...
#elif defined MACH_FPGA
	//TODO
	if ((inw(UART_BASE + 0x04) & 0x02) == 0)
		return -1;
	//c = inw(COM1 + 0x00) & 0xFF;
        
        c = inb(UART_BASE + thr) & 0xFF;
#endif
	if (c == 127) {
		c = '\b';
	}
